...
class HashProbeDetector:
    ...
    def __is_printable_ratio(data, threshold=0.90):
        if not data:
            return False
        printable = sum(
            chr(b) in string.printable and chr(b) not in "\x0b\x0c"
            for b in data
        )
    ...
    def __detect_base64(value):
        if not HashProbeDetector.__is_base64_charset(value):
            return None

        looks_like_hex = HashProbeDetector.__is_hex(value)
        ...
        looks_like_text = HashProbeDetector.__is_printable_ratio(decoded)
    ...
    def detect_hash(hash_value):
        ...
        base64_result = HashProbeDetector.__detect_base64(hash_value)
        if base64_result:
        ...
        for sig in HASH_SIGNATURES:
            ...
            if (
                charset == "hex"
                and HashProbeDetector.__is_hex(hash_value)
                or charset == "base64"
                and HashProbeDetector.__is_base64_charset(hash_value)
            ):
                score += 0.3
